...
import com.bsco.framework.image.ImageUtils.Position;
...
public class AverageImageScale {
	...
	public static void imageMark(File srcFile, File destFile, int minWidth,
			...
			Color color, int size, int alpha, int degree) throws IOException {
		...
		if (width <= minWidth || height <= minHeight) {
			...
			imageMark(imgBuff, width, height, pos, offsetX, offsetY, text,
					color, size, alpha, degree);
			writeFile(imgBuff, destFile);
			imgBuff = null;
		}
	}
	
	public static void imageMark(File srcFile, File destFile, String text,
			Color color, int size, int alpha, int degree) throws IOException {
		BufferedImage imgBuff = ImageIO.read(srcFile);
		imageMark(imgBuff, text, color, size, alpha, degree);
		writeFile(imgBuff, destFile);
		imgBuff = null;
	}
	...
	public static void imageMark(File srcFile, File destFile, int minWidth,
			...
			int alpha, int degree) throws IOException {
		BufferedImage imgBuff = ImageIO.read(srcFile);
		int width = imgBuff.getWidth();
		int height = imgBuff.getHeight();
		if (width <= minWidth || height <= minHeight) {
			...
			imageMark(imgBuff, width, height, pos, offsetX, offsetY, markFile, alpha, degree);
			writeFile(imgBuff, destFile);
			imgBuff = null;
		}
	}
	
	public static void imageMark(File srcFile, File destFile, File markFile,
			int alpha, int degree) throws IOException {
		BufferedImage imgBuff = ImageIO.read(srcFile);
		imageMark(imgBuff, markFile, alpha, degree);
		writeFile(imgBuff, destFile);
		imgBuff = null;
	}
	...
	private static void imageMark(BufferedImage imgBuff, int width, int height,
			...
			int size, int alpha, int degree) throws IOException {
		...
		// 设置水印旋转
		g.rotate(Math.toRadians(degree), (double) imgBuff.getWidth() / 2,
				(double) imgBuff.getHeight() / 2);
		g.setColor(color);
	...
	private static void imageMark(BufferedImage imgBuff, int width, int height,
			int pos, int offsetX, int offsetY, File markFile, int alpha,
	...
	private static void imageMark(BufferedImage imgBuff, String text, Color color,
			int size, int alpha, int degree) throws IOException {
		Graphics2D g = imgBuff.createGraphics();
		// 设置对线段的锯齿状边缘处理
		g.setRenderingHint(RenderingHints.KEY_INTERPOLATION,
		...
		g.rotate(Math.toRadians(degree), (double) imgBuff.getWidth() / 2,
				(double) imgBuff.getHeight() / 2);
		g.setColor(color);
		Font font = new Font("simsun", Font.BOLD, size);
		g.setFont(font);
		AlphaComposite a = AlphaComposite.getInstance(AlphaComposite.SRC_ATOP,
				(float) alpha / 100);
		g.setComposite(a);
		int posx = 0;
		int posy = 0;
		while(posy < (Math.max(imgBuff.getWidth(), imgBuff.getHeight()))) {
			posx += 200;
			g.drawString(text, posx, posy);
			if(posx > imgBuff.getWidth()) {
				posy += 200;
				posx = 0;
			}
		}
	...
	private static void imageMark(BufferedImage imgBuff, File markFile, int alpha,
			int degree) throws IOException {
		Graphics2D g = imgBuff.createGraphics();
		// 设置对线段的锯齿状边缘处理
		g.setRenderingHint(RenderingHints.KEY_INTERPOLATION,
				RenderingHints.VALUE_INTERPOLATION_BILINEAR);
		// 设置水印旋转
		g.rotate(Math.toRadians(degree), (double) imgBuff.getWidth() / 2, (double) imgBuff.getHeight() / 2);
		AlphaComposite a = AlphaComposite.getInstance(AlphaComposite.SRC_ATOP, (float) alpha / 100);
		g.setComposite(a);
		BufferedImage markBuffer = ImageIO.read(markFile);
		int posx = 0;
		int posy = 0;
		while(posy < (Math.max(imgBuff.getWidth(), imgBuff.getHeight()))) {
			posx += 200;
			g.drawImage(markBuffer, posx, posy, null);
			if(posx > imgBuff.getWidth()) {
				posy += 200;
				posx = 0;
			}
