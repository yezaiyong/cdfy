...
$(function() {
	...
	$okbtn.live('click', function() {
		...
		var $temp1 = $st.children("button").remove();
		content = $temp.html();
		...
		$.ajax( {
			...
			success : function(data) {
				if (data == "1") {
					...
					ZENG.msgbox.show("请登陆后再评论！", 3, 3000);
				}
			}
		});
	});
	...
	$(data.list)
			.each(
					function(i, per) {
						...
						html += ' <img class="q_namecard" src="' +comments.contextPath + per.PHOTO + '">'
						html += ' 	</a>'
						html += ' </div>'
						...
						if (per.LEVEL == "1") {
							...
							html += '回复';
							html += '<a class="c_tx q_namecard" target="_blank" href="javascript:;">' + per.PNAME + '</a>&nbsp;';
						}
						html += ':' + per.CONTENT + ''
